...
//图像初始化
void Image_SD_Init(void)
{
    int   fireres;
    uint8_t  myfilename[20];
    uint16_t imgsize[] = {CAMERA_H, CAMERA_W};
    uint32_t mybw;
    uint32_t Imag_num = 0;
    ...
    {
        Imag_num ++;
        sprintf(myfilename, "0:/%d.bin", Imag_num);
...
//sd保存图像
void Image_SD_Save(uint8_t * imgaddr)
{
#define SD_ONEWRITE_MAX 603  //一次写入的数据

    int32_t   fireres;
    uint32_t mybw;
    static uint8 time = 0;
    uint32_t size = CAMERA_SIZE;
    uint8_t  buff[3]={0xff,0x00,0xff};
    if(firesrc.fs != 0)
    {
         time ++;
         fireres = f_write(&firesrc,buff ,3 , &mybw);  //文件头
         fireres = f_write(&firesrc, imgaddr, size , &mybw); //数据  
     
        if(fireres != FR_OK)
        {
           f_close(&firesrc);    //关闭文件
            firesrc.fs = 0;
        }
